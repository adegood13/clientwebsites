/* Albuquerque Ink Tattoo — booking flow (simulated) */
(function () {
  ...
  if (params.get("artist")) {
    setTimeout(() => {
      ...
      const map = { "any artist": "Match me" };
      const lookup = map[target] || params.get("artist");
      const btn = $$("#artists .opt").find(b => b.dataset.artist.toLowerCase() === lookup.toLowerCase());
      if (btn) btn.click();
    }, 0);
  }
  ...
  function show() {
    ...
    next.textContent = state.step === 6 ? "Reserve →" : "Continue →";
  ...
  function canAdvance() {
    switch (state.step) {
      ...
      case 2: return !!state.artist;
      case 3: return !!$("#size").value && !!$("#placement").value.trim() && !!$("#desc").value.trim();
      ...
      case 5: {
        ...
        const age = $("#age").value;
        return fn && ln && ph.replace(/\D/g,"").length >= 10 && /\S+@\S+\.\S+/.test(em) && age === "yes";
      }
      case 6: return state.waiver;
    }
  }
  ...
  ["#size","#placement","#desc","#budget"].forEach(sel => {
    ...
    if (el) el.addEventListener("change", () => { next.disabled = !canAdvance(); });
  });
  ...
  function renderTimes(dow) {
    ...
    const slots = dow === 5
      ...
      : ["10:00am","11:00am","12:00pm","1:00pm","2:00pm","3:00pm","4:00pm","5:00pm","6:00pm","7:00pm","8:00pm","9:00pm"];
  ...
  function reserve() {
    ...
    sessionStorage.setItem("abq_booking", JSON.stringify(payload));
