/* Golden Auto Detailing — booking flow (simulated) */
(function () {
  ...
  // pre-select package from URL ?pkg=...
  const params = new URLSearchParams(window.location.search);
  if (params.get("pkg")) {
    const target = params.get("pkg").toLowerCase();
  ...
  function renderTimes() {
    ...
    const slots = ["9:00am","10:00am","11:00am","12:00pm","1:00pm","2:00pm","3:00pm","4:00pm","5:00pm"];
    const seed = state.date.split("-").reduce((a,b)=>a+ +b,0);
    const taken = new Set();
    for (let i=0;i<2+(seed%3);i++) taken.add(slots[(seed*(i+5))%slots.length]);
    slots.forEach(s => {
  ...
  function renderSummary() {
    ...
    $("#summary").innerHTML = `
      ...
      <dl>
        ...
        ${s.deposit ? `<div><dt>Deposit (today)</dt><dd>−$${dep}</dd></div><div><dt>Balance on completion</dt><dd>$${balance}</dd></div>` : ""}
  ...
  function reserve() {
    ...
    sessionStorage.setItem("golden_booking", JSON.stringify(payload));
